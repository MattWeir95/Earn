<div x-data="{isOpen: false}" x-cloak x-show="isOpen" @keydown.escape.window="isOpen = false" 
        class="absolute inset-10 z-10 flex items-center justify-center"
        ...
        x-init="setTimeout(() => isOpen = true, 1000)">
        <div class="bg-white border-2 border-gray-200 rounded p-2">
            ...
            <div>
                ...
                <p class="text-xl mt-6">Total: ${{ $total }}</p>
                <p class="text-md mt-2">Active Period: {{ $start_time }}-{{ $end_time }}</p>
            </div>
